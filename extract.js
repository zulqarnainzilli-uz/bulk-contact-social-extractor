...
async function extractBulkData(rawInput) {
  ...
  for (let i = 0; i < urls.length; i++) {
    const row = await processWebsite(urls[i]);
    results.push(row);
  }
...
async function fetchPage(url) {
  try {
    const response = await fetch(url, {
      redirect: 'follow',
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; BulkExtractorBot/2.0; +https://vercel.app/)'
      }
    });
    ...
    return { ok: false, url, code: 0, error: String(e) };
  }
}
